#!/bin/bash
# nohup 
# nohup bash run_swift_S1_Omni_chem.sh  > /nfs-13/wuxiaoyu/科学Omni/Sequence_tokenizer/msswift/output/nohub_log/S1_Omni_vocabextension_chem_test_$(date +%Y%m%d_%H%M%S).log 2>&1 &
...
# 公共脚本目录
SCRIPT_DIR="/nfs-13/wuxiaoyu/科学Omni/Sequence_tokenizer/msswift/my_train_scripts/ext_sft"
...
for task_entry in "${TRAIN_TASKS[@]}"; do
    ...
    # 生成带任务名的日志文件名
    LOG_FILE="/nfs-13/wuxiaoyu/科学Omni/Sequence_tokenizer/msswift/output/log/train_${TASK_NAME}_$(date +%Y%m%d_%H%M%S).log"
